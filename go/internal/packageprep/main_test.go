...
func TestPrepareWritesDeterministicManifestAndTokenFreeArchive(t *testing.T) {
	...
	for _, file := range archive.File {
		...
		if !strings.HasPrefix(file.Name, "golang.cekat.ai/event-sdk@v0.3.0/") {
		...
		reader.Close()
		if _, err := os.Stat(filepath.Join(moduleRoot, filepath.FromSlash(strings.TrimPrefix(file.Name, "golang.cekat.ai/event-sdk@v0.3.0/")))); err != nil {
...
func TestPrepareWritesOneArchivePerPublishableModule(t *testing.T) {
	...
	want := map[string][]string{
		"cekat-event-sdk-go-middleware-gin-v0.3.0.zip": {
			"golang.cekat.ai/event-sdk/middleware/gin@v0.3.0/go.mod",
			"golang.cekat.ai/event-sdk/middleware/gin@v0.3.0/go.sum",
			"golang.cekat.ai/event-sdk/middleware/gin@v0.3.0/middleware.go",
		},
		"cekat-event-sdk-go-v0.3.0.zip": {
			"golang.cekat.ai/event-sdk@v0.3.0/README.md",
			"golang.cekat.ai/event-sdk@v0.3.0/client.go",
			"golang.cekat.ai/event-sdk@v0.3.0/go.mod",
			"golang.cekat.ai/event-sdk@v0.3.0/internal/shared/shared.go",
			"golang.cekat.ai/event-sdk@v0.3.0/nested/source.go",
		},
	}
	if len(manifest.Artifacts) != len(want) || manifest.Artifacts[0].Path != "cekat-event-sdk-go-middleware-gin-v0.3.0.zip" {
