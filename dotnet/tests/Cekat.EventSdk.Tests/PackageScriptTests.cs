...
public sealed class PackageScriptTests
{
    ...
    public async Task PackageScript_rejects_invalid_arguments_before_running_checks()
    {
        ...
        [
            ...
            ["--version", "0.3.0", "--output", $"{temp}/../tmp"],
            ["--version", "0.3.0", "--output", dirty.FullName],
            ["--version", "0.3.0", "--output", Path.Combine(dirty.FullName, "stale.nupkg")],
            ["--version", "0.3.0", "--output", Path.Combine(DotnetDirectory, "artifacts-output")],
        ];
        try
        {
