...
`order_paid` takes two required arguments in addition to the event: a finite number `amount` and a string `currency`. The SDK sends them as `properties.amount` and `properties.currency`, merged with any caller properties, and applies the ordinary properties rules to `amount` (so integral amounts must be within the safe-integer range). `currency` must be nonblank after trimming and is otherwise transmitted unchanged; the SDK does not validate currency codes. Caller properties that already contain an `amount` or `currency` key are a validation error rather than being silently overwritten. Non-finite amounts are a validation error. Fixtures declare these arguments as `operation.amount` and `operation.currency`, which are required for `order_paid` and forbidden for every other operation. The other four operations take no additional arguments.
