...
- `OrderPaid(event)` → `event_key: order_paid`, `is_common: true`
...
An event input contains optional `email`, `phone_number`, `contact_name`, `properties`, and an explicit optional `visitor_id`. The explicit visitor ID is an escape hatch for unsupported frameworks, tests, and non-HTTP execution.
...
- request `timeout`, with one documented default selected during implementation
...
- **Go:** `context.Context`. Generic extraction returns an enriched context/request. Tracking methods accept a context. Framework middleware ensures handlers receive an enriched request context.
...
├── go/
│   ├── cekat/
│   └── middleware/{nethttp,gin,echo,fiber,chi}/
├── node/
│   ├── src/
...
Framework adapters stay thin. Validation, models, HTTP submission, retry behavior, response decoding, and errors belong in each language core.
...
It does not automatically retry `400`, `401`, `404`, or other statuses unless a future authoritative API contract marks them transient. Backoff is exponential with jitter and is interruptible by the language’s cancellation mechanism. The implementation plan will define exact delay bounds and timeout defaults uniformly where practical.

The API offers no idempotency key. Retrying a request whose outcome is unknown can create duplicate events. SDK documentation and typed errors must communicate this. Caller event or deduplication IDs may be included in `properties`, but the SDK must not claim server enforcement.
...
- exactly two default retries for retryable failures;
