...
> **Implementation notes (2026-09-14) — Tasks 1 and 6 as built.** `ci/package-manifest.schema.json`, `scripts/validate-package-manifest.py` (standard library; single manifest or `--all`, plus `--markdown` for the job summary), `scripts/package-readiness.sh`, `scripts/tests/test_package_manifest.py`, and `.github/workflows/release-readiness.yml` exist. `--all` keeps packaging the remaining languages after a failure so every problem is reported, and runs aggregate validation only when all seven succeed; package scripts run in the background so interrupts reach them. The workflow's preflight runs only the root script tests (the compatibility matrix and doc audit from Tasks 3 and 4 do not exist yet), its toolchains mirror the current rows of ci.yml on the self-hosted `platform-ci-aws` runner, and actions use major tags like ci.yml rather than commit SHAs. The contract job in ci.yml also runs the root script tests. All seven real package scripts were run through the wrapper in current-toolchain containers on 2026-09-14 and their manifests validated as an aggregate. docs/release-checklist.md was not created; the workflow's manual-gates job and the root README list the release-owner steps.

> **Implementation notes (2026-09-14) — Tasks 2 and 3 as built.** `scripts/conformance.sh` is a thin entry point to `scripts/conformance.py` (standard library): one private mock per language in its own session, readiness and reset checks, exactly the four `CEKAT_CONFORMANCE_*` variables, token redaction, TERM-then-KILL cleanup, a signal-only interrupt handler, and exact accounting (every fixture once as `passed`, or `not_applicable` only when the fixture declares the language; `runtime_deviation` records count as passed and are reported). Processes a runner leaves behind are stopped with a warning rather than failing the language, because the .NET SDK's build servers intentionally outlive `dotnet run`. `scripts/tests/test_conformance_contract.py` uses the real Go mock with probe runners. `ci/compatibility-matrix.json` is per language (runtimes with declared floors, CI job/key values, minimum and current profiles with observed exact versions, release-readiness value, frameworks, end-of-support dates) rather than fourteen flat entries, and ci.yml keeps its hand-written matrices; `scripts/validate-compatibility-matrix.py` detects drift against package metadata, ci.yml, release-readiness.yml, and evidence documents instead of generating the CI matrix. Time gates (`--as-of`, `--max-age-days`) run only in release-readiness preflight. There is no `--github-matrix` output and runner labels are not validated (CI uses the self-hosted `platform-ci-aws` label).

> **Implementation notes (2026-09-14) — Task 4 as built.** `docs/sdk-contract.md`, `docs/visitor-propagation.md`, `docs/retry-and-error-semantics.md`, and `docs/release-checklist.md` describe the amended contract (3-second timeout, retries on 429/500/502/503/504, `Retry-After`, `event_id`, order paid arguments, cancellation per language, the Bun runtime limitation) with per-language name tables; `docs/compatibility.md` is generated from `ci/compatibility-matrix.json` by `scripts/audit-docs.py --write-compatibility`. `scripts/audit-docs.py` (standard library) audits the root README, those five documents, the conformance guide, and the seven language READMEs: document set and root links, relative links and anchors, unfinished markers, absolute local paths, token-like secrets, publication commands and claims, exact origin/path/header/cookie spellings, positive durability or exactly-once claims outside negated sentences and code, stale 10-second timeouts or 500-only retry claims, required terms per document, each README naming its matrix runtime floor and frameworks, and a current compatibility page. Plans are not audited. The audit found and fixed two README gaps (Node lacked the untrusted-visitor warning, .NET did not name `event_id`). `scripts/tests/test_audit_docs.py` copies the repository and breaks one thing per case. The audit runs in the ci.yml contract job and the release-readiness preflight.
