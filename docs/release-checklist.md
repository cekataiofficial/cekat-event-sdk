...
4. Download the `cekat-event-sdk-0.3.0-<language>` artifacts you will publish and keep the hashes with the release record.
...
- [ ] **Tags.** Go modules are tagged with their full directory path (`go/v0.3.0`, `go/middleware/gin/v0.3.0`); pushing the core tag creates the adapter tags. The npm package is tagged `node/v0.3.0`.
...
2. Merge, wait for `CI required`, then push the core tag on the release commit:
   ...
   ```sh
   git fetch origin && git tag go/v0.3.0 origin/main && git push origin go/v0.3.0
   ```
...
5. Confirm from a clean directory that the modules resolve:
   ...
   ```sh
   go list -m golang.cekat.ai/event-sdk@v0.3.0 golang.cekat.ai/event-sdk/middleware/gin@v0.3.0
   ```
