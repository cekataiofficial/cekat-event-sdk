...
async function request(app: express.Express, path: string, headers: Record<string, string> = {}, init: RequestInit = {}): Promise<Response> {
...
describe('Express visitor middleware', () => {
  ...
  it('keeps the visitor through body parsing installed after the middleware on parallel POST requests', async () => {
    const app = express();
    app.use(visitorMiddleware());
    app.use(express.json());
    app.post('/orders', async (req, response) => {
      await new Promise<void>((resolve) => setTimeout(resolve, Math.floor(Math.random() * 5)));
      response.json({ visitorId: currentVisitorId() ?? null, orderId: (req.body as { orderId: string }).orderId });
    });

    const visitorIds = Array.from({ length: 10 }, (_, index) => `express-post-${index}`);
